...
//AA Believed to be done. Uses the method of postOrderTraversal to go reach all of the leaves and delete them and work back
template<class ItemType>
...
int BST_312 <ItemType>::countNodes(TreeNode* t) const
{
   ...
   numNodes = numNodes + countNodes(t->left);
   numNodes = numNodes + countNodes(t->right);
   return numNodes;
}
...
//AA Compiles, believed to be done. Calls a traversal to make a vector then uses the vectorfind to see if item is in the vector, if so it returns true.
template<class ItemType>
